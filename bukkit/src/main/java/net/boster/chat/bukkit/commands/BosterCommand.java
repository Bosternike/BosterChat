...
public abstract class BosterCommand extends Command implements PluginIdentifiableCommand {
    ...
    @Getter @NotNull protected final BosterChatBukkit plugin;
    @Getter @Setter @Nullable protected List<String> subCommands;

    public BosterCommand(@NotNull BosterChatBukkit plugin, @NotNull String name, @NotNull List<String> aliases) {
        super(name);
        this.setAliases(aliases);
        this.plugin = plugin;
    }

    public BosterCommand(@NotNull BosterChatBukkit plugin, @NotNull String name, @NotNull String... aliases) {
    ...
    public boolean checkPlayer(CommandSender sender) {
        if(sender instanceof Player) {
            ...
            sender.sendMessage("§b[§dBosterChat§b] §cYou must be a player to perform this command.");
            return false;
        }
    }
